...
const CarDetail = () => {
  ...
  useEffect(() => {
    const fetchCarDetail = async () => {
      try {
        const response = await axios.get(`https://car-dealership-server-6i5y.onrender.com/car/car/${id}`);
        setCar(response.data);

        // Fetch reviews for the car based on car ID
        const reviewsResponse = await axios.get(`https://car-dealership-server-6i5y.onrender.com/car/car/review/${id}`);
  ...
  const handleAddToCart = async carId => {
    try {
      const response = await axios.post(
        'https://car-dealership-server-6i5y.onrender.com/cart/additem',
        { carid: carId },
  ...
  const handleRemoveFromCart = async carId => {
    try {
      const response = await axios.delete('https://car-dealership-server-6i5y.onrender.com/cart/deleteitem', {
        headers: { Authorization: `Bearer ${token}` },
        data: { itemId: carId }
      });
      setCartItems(response.data);
            ...
            <div className="grid grid-cols-3 gap-4">
              {car.carImages &&
                car.carImages.map((image, index) => (
                  ...
                  >
                    <img
                      src={`https://car-dealership-server-6i5y.onrender.com/${image}`}
                ...
                <div key={review._id} className="bg-gray-800 p-4 rounded-lg">
                  ...
                  <div className="flex space-x-2">
                    {review.images.map((image, index) => (
                      <img
                        key={index}
                        src={`https://car-dealership-server-6i5y.onrender.com/${image[index]}`}
