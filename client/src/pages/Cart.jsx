...
const Cart = () => {
  ...
  const fetchCartItems = async () => {
    try {
      const response = await axios.get("https://car-dealership-server-6i5y.onrender.com/cart/getitem", {
        headers: {
  ...
  const handleBuy =  () => {
    ...
    //   const response = await axios.post("https://car-dealership-server-6i5y.onrender.com/cart/buy", { ids: carIds }, {
    //     headers: {
    //       Authorization: `Bearer ${token}`,
    //     },
    //   });
  ...
  const handleRemoveItem = async (id) => {
    try {
      await axios.delete(`https://car-dealership-server-6i5y.onrender.com/cart/deleteitem`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        data: { itemId: id },
          ...
          <>
            ...
            <ul className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
              {cartItems.map((item) => (
                ...
                >
                  <img
                    src={`https://car-dealership-server-6i5y.onrender.com/${item.carImages[0]}`}
